...
def get_basketball_data():
    ...
    for i in range(len(standings["response"][0])):
        ...
        if standings['response'][0][i]['group']['name'] == "Western Conference":
            if "Western Conference" not in standings_dict[leagueName]:
                standings_dict[leagueName]["Western Conference"] = {}
            
            standings_dict[leagueName]["Western Conference"][teamName] = {}
            standings_dict[leagueName]["Western Conference"][teamName]["position"] = position 
            standings_dict[leagueName]["Western Conference"][teamName]["for"] = forPoints
            standings_dict[leagueName]["Western Conference"][teamName]['id'] = leagueID
            standings_dict[leagueName]["Western Conference"][teamName]["against"] = against
